...
document.ondragenter = (e) => {
  if (isEditorMode() || isPicMode() || window.ro) {
    return;
  }
  cancelDefault(e);
...
document.ondragstart = (e) => {
  draggingSrc = e.target.innerHTML;
};
...
document.ondrop = (e) => {
  ...
  if (draggingSrc && t) {
    ...
    ensureMove() ||
      mvCall(prependPath(draggingSrc), prependPath(dest), refresh);
    // ... or upload
...
window.quitAll = () =>
  helpOff() || picsOff() || videosOff() || padOff() || pdfOff();
...
document.body.addEventListener(
  ...
  (e) => {
    ...
    if (isHelpMode()) {
      return prevent(e) || window.quitAll();
    }
    ...
    if (e.code.includes('Key') && !e.ctrlKey && !e.metaKey) {
      ...
      const a =
        allA.find((el) =>
          el.innerText.toLocaleLowerCase().startsWith(typedPath),
        ) ||
        allA.find((el) => el.innerText.toLocaleLowerCase().includes(typedPath));
      if (!a) {
        return;
      }
